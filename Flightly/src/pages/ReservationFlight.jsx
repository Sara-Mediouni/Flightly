...
const FlightReservation = () => {
    ...
    const handleSubmit=async (e) => {
        ...
        if (flightData.personCount.Adultes + flightData.personCount.Enfants > flight?.availableSeats) {
             toast.error('Not enough available seats for the selected number of passengers.');
            return;
        }
  ...
  <div className="w-1/2 pr-8 max-w-[300px] overflow-y-auto">
    <h2 className="md:text-3xl font-semibold text-violet900 mb-4">Book Your Flight</h2>
                    <p className="text-lg text-gray-700 mb-4">
                        Choose from a variety of flights and enjoy the best services during your journey. We offer different classes and comfortable flight options to ensure a great experience.
                    </p>
                    <p className="text-md text-gray-600">
                        <strong>Traveling From:</strong> {flight?.departurePlace}, {flight?.departureAirport} 
                    </p>
                    <p className="text-md text-gray-600">
                        <strong>Destination:</strong> {flight?.returnPlace}, {flight?.returnAirport}
                    </p>
                    <p className="text-md text-gray-600">
            ...
            <div className="flex flex-col items-center justify-center space-y-6">
                <div className="w-full max-w-4xl bg-violet-300 rounded-xl shadow-xl p-8">
                    <h1 className="text-3xl font-bold text-center text-violet-900 mb-8">Flight Reservation</h1>
                    ...
                    <form onSubmit={(e)=>{  
                        ...
                        <div className="mb-8">
  <h2 className="text-xl font-semibold mb-4">Select Your Flight Class</h2>
  <div className="grid md:grid-cols-2 gap-6">
    {flight?.classes.map((cls, index) => (
      <div
        key={index}
        name="flightClass"
        className={`flex items-center justify-between p-4 border rounded-md shadow-md cursor-pointer ${flightData.flightClass === cls.name ? 'bg-violet-100' : 'bg-white'}`}
        onClick={() => handleFlightSelection(cls.name,cls.price)}
      >
        <div>
          <h3 className="font-semibold text-violet-900 capitalize">{cls.name} Class</h3>
          <p className="text-sm text-gray-700">{cls.availableSeats} Seats</p>
        </div>
        <p className="text-lg font-bold text-violet-700">${cls.price} / Ticket</p>
      </div>
    ))}
  </div>
</div>
                        ...
                        {flightData.personCount.Enfants > 0 && (
                            <div className="mb-8">
                                <h2 className="text-xl font-semibold mb-4">Enter Children's Ages</h2>
                                {[...Array(parseInt(flightData.personCount.Enfants))].map((_, index) => (
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
  ...
  Total Price: <span className="text-violet900">{flightData.TotalPrice} USD</span>
</div>
                        {/* Submit Button */}
