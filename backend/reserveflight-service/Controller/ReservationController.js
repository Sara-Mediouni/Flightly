...
const placeOrder = async (req, res) => {
  try {
    ...
    try{
     ...
     try{
      const getFlight = await axios.get(`http://localhost:4000/flight/flight/${flightId}`)
      Flight=getFlight.data.flightData
    }
    catch(error){
    ...
    try{
      const updateUser= await axios.put(`http://localhost:4000/user/user/updateuser/${userId}`,{ reservations: newOrder._id })   
      }
      catch(error)
...
const verifyOrder = async (req, res) => {
  ...
  try {
    if (success === "true") {
      ...
      try{
      const flightf = await axios.get(`http://localhost:4000/flight/flight/${order.flight}`)
  ...
  try {
    ...
    const orders = await ReserveModel.find({ user: req.params.userId });
      ...
      orders.map( async(order)=>{
        const userResponse=await axios.get(`http://localhost:4000/user/user/getuser/${order.user}`)
        return({
          ...order.toObject(),
          user:userResponse.data.user
        })
